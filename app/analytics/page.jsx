...
const label = Array.isArray(finalPrediction) ? finalPrediction.join(", ") : finalPrediction;
...
backgroundColor: ["#34d399", "#60a5fa", "#fbbf24", "#f472b6", "#a78bfa"],
},
],
};
...
const mostCommonDisease = sortedClasses.length > 0 ? sortedClasses[0][0] : "None";

const ageDiseaseInsights = Object.entries(diseaseByAge).map(([age, diseases]) => {
...
<h1 className="text-3xl font-bold mb-4">📊 Student Analytics Dashboard</h1>
<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div className="bg-gray-600 p-4 rounded shadow col-span-full">
      <h2 className="text-xl font-semibold mb-2">Age-wise Disease Distribution</h2>
      <Bar data={stackedData} options={{ responsive: true, plugins: { legend: { position: "top" } }, scales: { x: { stacked: true }, y: { stacked: true } } }} />
    </div>
  </div>

  <div className="bg-gray-600 p-4 rounded shadow">
    <h2 className="text-xl font-semibold mb-2">Insights</h2>
    <p>🧠 Most Common Detected Condition: <strong>{mostCommonDisease}</strong></p>
    <p>🧩 Students with both ADHD & Dyslexia: <strong>{comorbidCount["ADHD & Dyslexia"]}</strong></p>
    ...
    <div className="mt-4">
      <h3 className="text-lg font-semibold">Top Risk Features (Score &lt; 50):</h3>
      <p>🔹 ADHD: {Object.entries(riskFeatures.ADHD).map(([k, v]) => `${k} (${v})`).join(", ") || "None"}</p>
      <p>🔹 Dyslexia: {Object.entries(riskFeatures.Dyslexia).map(([k, v]) => `${k} (${v})`).join(", ") || "None"}</p>
    </div>
  </div>
</div>
);
